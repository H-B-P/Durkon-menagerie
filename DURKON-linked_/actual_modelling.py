...
def train_model(inputDf, target, nrounds, lr, startingModel, weights=None, grad=calculus.Gauss_grad, link=calculus.Unity_link, linkgrad=calculus.Unity_link_grad):
 ...
 for i in range(nrounds):
  ...
  op = apply_model.pred_from_effects(model["BASE_VALUE"], len(inputDf), contEffects, catEffects)
  pred = op.apply(link)
  
  gradient = grad(pred, np.array(inputDf[target])) #d(Loss)/d(pred)
  lgradient = linkgrad(op) #d(pred)/d(op)
  ...
  for col in model["conts"]:
   ...
   finalGradients = np.matmul(np.array(gradient*lgradient),cowrd[col]) #d(Loss)/d(pt) = d(Loss)/d(pred) * d(pred)/d(op) * d(op)/d(pt)
  ...
  for col in model["cats"]:
   ...
   finalGradients = np.matmul(np.array(gradient*lgradient),cawrd[col]) #d(Loss)/d(pt) = d(Loss)/d(pred) * d(pred)/d(op) * d(op)/d(pt)
...
if __name__ == '__main__':
 ...
 #df = pd.DataFrame({"x":[1,2,3,4,5,6,7,8,9],"y":[1,4,9,16,25,36,49,64,81]})
 #model = {"BASE_VALUE":3.0,"conts":{"x":[[1,5],[5,5],[9,5]]}, "cats":[]}
 #newModel = train_model(df, "y",100, 0.004, model,weights=[1,2,3,4,5,6,7,8,9], link = calculus.Root_link, linkgrad = calculus.Root_link_grad)
 #apply_model.explain(newModel)
 
 df = pd.DataFrame({"x":[1,5,5,9,9,9,9],"y":[1,0,1,0,0,0,1]})
 model = {"BASE_VALUE":0,"conts":{"x":[[1,0],[5,0],[9,0]]}, "cats":[]}
 newModel = train_model(df, "y",1000, 0.4, model,weights=[1,1,1,1,1,1,1], grad = calculus.Logit_grad, link = calculus.Logit_link, linkgrad = calculus.Logit_link_grad)
 apply_model.explain(newModel)