 ...
 for i in range(nrounds):
  ...
  for m in range(len(models)):
   ...
   for col in model["conts"]:
    ...
    print(gradient)
    print(contWReleDictList)
    
    finalGradients = np.matmul(gradient,contWReleDictList[m][col]) #d(Loss)/d(pt) = d(Loss)/d(pred) * d(pred)/d(pt)
   ...
   for col in model["cats"]:
    ...
    finalGradients = np.matmul(gradient,catWReleDictList[m][col]) #d(Loss)/d(pt) = d(Loss)/d(pred) * d(pred)/d(pt)
...
if __name__ == '__main__':
 ...
 models = [{"BASE_VALUE":3.0,"conts":{"x":[[1,5],[5,5],[9,5]]}, "cats":[]}]
