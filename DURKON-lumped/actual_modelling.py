...
def train_model(inputDf, target, nrounds, lr, startingModel, llr=0, weights=None, grad=calculus.Gauss_grad):
 ...
 for i in range(nrounds):
  ...
  print('adjust base value and lump')
  if 'LUMP' in model:
   antilump = gradient*pred/model['BASE_VALUE'] #d(Loss)/d(BV) = d(Loss)/d(pred) * d(pred)/d(BV)
   
   lumpshift = sum(weights*(gradient - antilump))/sw #d(Loss)/d(Lump) - d(Loss)/d(BV))
   
   model['LUMP']-=lumpshift*llr
   model['BASE_VALUE']+=lumpshift*llr
...
if __name__ == '__main__':
 df = pd.DataFrame({"x1":[1,1,1,2,2,2,3,3,3],"x2":[1,2,3,1,2,3,1,2,3],"y":[2,3,4,3,5,7,4,7,10]})
 model = {"BASE_VALUE":5.0,"LUMP":0,"conts":{"x1":[[1,1],[2,1],[3,1]],"x2":[[1,1],[2,1],[3,1]]}, "cats":[]}
 
 model = train_model(df, "y",100000, 0.01, model, 0.01, weights=[1,2,3,4,5,6,7,8,9])
 apply_model.explain(model)
 
 print(apply_model.predict(df,model))
