...
def train_model(inputDf, target, nrounds, lr, startingModel, weights=None, grad=calculus.Gauss_grad):
 ...
 for i in range(nrounds):
  ...
  for col in model["conts"]:
   ...
   finalGradients = np.matmul(gradient,cowrd[col]) #d(Loss)/d(pt) = d(Loss)/d(pred) * d(op)/d(pt)
  ...
  for col in model["cats"]:
   ...
   finalGradients = np.matmul(gradient,cawrd[col]) #d(Loss)/d(pt) = d(Loss)/d(pred) * d(pred)/d(pt)
